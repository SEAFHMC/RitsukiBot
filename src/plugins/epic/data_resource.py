...
async def get_game_list_online() -> List:
    try:
        ...
        present = epicgames(
            list(filter(lambda x: x["promotions"]["promotionalOffers"], game_list))[0]
        )
        next = epicgames(
            list(
                filter(
                    lambda x: x["promotions"]["upcomingPromotionalOffers"], game_list
                )
            )[0]
        )
...
async def make_msg():
    ...
    msg = (
        ...
        + "\n"
        + "截止时间："
        + datetime.fromisoformat(present_endDate).strftime("%Y.%m.%d %H:%M")
    )
    if next:
        msg += f"\n下次可白嫖：{next.title}"
    return msg
